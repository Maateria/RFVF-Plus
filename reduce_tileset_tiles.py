...
Usage:
    python reduce_tileset_tiles.py <tileset_path> [--max-tiles 384]

Exemple:
    python reduce_tileset_tiles.py data/tilesets/secondary/lilycove
"""
...
def create_reduced_tileset(
    ...
    for old_id, new_id in tile_mapping.items():
        ...
        # Position du nouveau tile
        new_x = (new_id % new_tiles_per_row) * tile_size
        new_y = (new_id // new_tiles_per_row) * tile_size

        # Copier le tile
        tile = img.crop((old_x, old_y, old_x + tile_size, old_y + tile_size))
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python reduce_tileset_tiles.py <tileset_path> [--max-tiles 384]")
        print("\nExemple:")
        print("  python reduce_tileset_tiles.py data/tilesets/secondary/lilycove")
        sys.exit(1)
    ...
    new_tiles_png = tiles_png_path
    create_reduced_tileset(tiles_png_path.with_suffix(tiles_png_path.suffix + backup_suffix),
                          new_tiles_png, tile_mapping)
    print()
